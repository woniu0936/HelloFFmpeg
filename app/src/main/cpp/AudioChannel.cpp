...
void AudioChannel::decode() {
    ...
    while (isPlaying) {
        //取出一个数据包
        int ret = avPackets.deQueue(packet);
        if (!isPlaying) {
            break;
        }
        //取出失败
...
int AudioChannel::getPcm() {
    int data_size = 0;
    AVFrame *frame;
    int ret = avFrames.deQueue(frame);
    if (!isPlaying) {
        if (ret) {
...
/**
 * 播放解码后的音频流（代码参见：https://github.com/android/ndk-samples/tree/main/native-audio）
 */
